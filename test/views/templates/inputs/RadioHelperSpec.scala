...
class RadioHelperSpec extends TemplateBaseSpec {
  ...
  def generateExpectedRadioMarkup(value: String, display: String, checked: Boolean = false): String =
    s"""
       ...
       |    <input type="radio" id="$value" name="$fieldName" value="$value"${if (checked) " checked" else ""}>
  ...
  "Calling the radio helper with no choice pre-selected" should {
    ...
    "render the choices as radio buttons" in {
      val field: Field = Field(WhatToDoForm.whatToDoForm, fieldName, Seq(), None, Seq(), None)
      val expectedMarkup = Html(
        s"""
           |  <div>
           |    <fieldset>
           |      <legend>
           |        <h1 id="page-heading" class="heading-large">${title("")}</h1>
           |      </legend>
           |
           |      <div>
      ...
      val markup = radioHelper(field, choices, title(""))
  ...
  "Calling the radio group helper with a choice pre-selected" should {
    ...
    "render a list of radio options with one pre-checked" in {
      val field: Field = Field(WhatToDoForm.whatToDoForm, fieldName, Seq(), None, Seq(), Some("value2"))
      val expectedMarkup = Html(
        s"""
           |  <div>
           |     <fieldset>
  ...
  "Calling the radio group helper with an error" should {
    ...
    "render an error" in {
      val errorMessage = "Error message"
      val field: Field = Field(WhatToDoForm.whatToDoForm, fieldName, Seq(), None, Seq(FormError("text", errorMessage)), None)
      val expectedMarkup = Html(
        s"""
